...
void update_display(struct SensorData *data)
{
    ...
    ssd1306_clear_screen(&oled_display, false); // Clear the screen
...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        if (button1flag == 1)
        {
            printf("button1 pressed, system restart\n");
            esp_restart();
            button1flag = 0;
        }
        if (button2flag == 1)
        {
            printf("button2 pressed, now sensor\n");
            struct SoilData soil = soilPoll();
            data.soil = soil;
            struct AirData air = tempHumidPoll();
            data.air = air;
